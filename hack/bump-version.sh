...
# strings across docs/, helm-charts/, Makefile, .job.yml, and READMEs for a
# DCM release. Does not touch git (no branch/commit/tag) — intended for CICD
...
README_AINIC="README_AINIC.md"
RELEASENOTES_MD="docs/releasenotes.md"

for f in "$CHART_YAML" "$MAKEFILE" "$JOB_YML" "$CONF_PY" "$INDEX_MD" \
	"$HELM_MD" "$EXAMPLE_YAML" "$README_AINIC" "$RELEASENOTES_MD"; do
	require_file "$f"
done
...
# .job.yml: chart artifact path (tag)
sed -i "s|device-config-manager-charts-v[0-9]\+\.[0-9]\+\.[0-9]\+\.tgz|device-config-manager-charts-${VERSION_TAG}.tgz|g" \
	"$JOB_YML"
track_change "$JOB_YML"
...
# README_AINIC.md: scoped to the two release-version lines only —
# `--set image.tag=vX.Y.Z` and the `image.tag` table row. Leaves the
# custom-tag example and the `VERSION=v2.0.0 make` example untouched.
sed -i -e "s#--set image.tag=v[0-9]\+\.[0-9]\+\.[0-9]\+#--set image.tag=${VERSION_TAG}#g" \
	-e "s#\(\`image.tag\` | \)\`v[0-9]\+\.[0-9]\+\.[0-9]\+\`#\1\`${VERSION_TAG}\`#" \
	"$README_AINIC"
track_change "$README_AINIC"
